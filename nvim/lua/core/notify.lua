--- This module implements a pausable `vim.notify` in order to
...
function M.notify(message, level, opts)
  if M.is_paused() then
    ...
    if type(pos) ~= "number" or not notifications[pos] then
      pos = #notifications + 1
    end
    if opts then
      opts.replace = nil
...
--- Remove `notify` utilities and restore original `vim.notify`
function M.restore()
